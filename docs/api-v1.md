...
| `POST` | `/api/v1/integrations/google-drive/callback` | 물리 자료 화면이 전달한 일회용 state·code와 PKCE를 검증하고 암호화된 장기 연결 토큰 저장 |
...
- Google callback 주소는 `${APP_ORIGIN}/physics/library`로 고정한다. 화면은 일회용 `state`와 `code`를 즉시 모듈 메모리로 옮기고 주소에서 제거한 뒤, 같은 origin의 callback API에 POST한다. 전역 Referrer Policy는 `no-referrer`다. `state`는 소유자별·10분·일회용이고 PKCE `S256`을 사용한다.
...
- 기존 Drive PDF 선택, 페이지 추출, 파일별 AI 허용 API는 후속 단계다.
